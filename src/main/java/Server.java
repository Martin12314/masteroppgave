...
public class Server {
    ...
    // FIX: The SW's hardcoded public key (client-req-1). Pre-registering this
    // at startup means requests signed by the SW are recognised immediately
    // without requiring a dynamic /req-key/register call.
    private static final String SW_CLIENT_KEY_JWK =
            "{\"kty\":\"RSA\",\"kid\":\"client-req-1\",\"use\":\"sig\",\"alg\":\"PS256\"," +
            "\"n\":\"u31RCl3cGubeBaacyzc00o5jLnBlfNefG1sKCVQdNSSDJdOFJ96oS7oIlL31qEQM1hDCH3MIqZH" +
            "wHzgTpAXfLZDaJme-GPDT8tJCj8jfbCMVpeGQMbV9N6N7nlQOk8MJEK8TeS8fK3I4M2QiZngbhKDEhNAe" +
            "jiaMvuqWd60RAM1UwsEY_XBNWPFO3Ig9uROrDyfQdu1LCD9tFsBzVBn-O-eFdEVa0wO4LMdQmGgEl6xFfE" +
            "nNC36TFR7_TZJEuHuBS_rLtUVIy1arPUInN1snPkcBzadklNYpvT1szuMghVJES_-HQKgs_19KdA-4f1ne" +
            "zprTBQYrtMyBfGnCoM8JCQ\",\"e\":\"AQAB\"}";
    ...
    public static void main(String[] args) throws Exception {
        ...
        // FIX: pre-register the SW's fixed public key so the server can verify
        // requests from the SW without requiring a dynamic registration call.
        preRegisterSwClientKey();
    ...
    // FIX: pre-registers the SW's known public key so verifyRequest succeeds
    // for the SW's hard-coded client-req-1 key without a dynamic registration.
    private static void preRegisterSwClientKey() {
        try {
            PublicJsonWebKey pjwk = PublicJsonWebKey.Factory.newPublicJwk(SW_CLIENT_KEY_JWK);
            long now = System.currentTimeMillis() / 1000L;

            RegisteredClientKey reg = new RegisteredClientKey();
            reg.clientKeyId = "client-req-1";
            reg.publicKey = pjwk.getKey();
            reg.created = now;
            reg.expires = now + (365L * 24 * 3600); // 1 year
            reg.swBuild = "preregistered";
            reg.registeredAtMs = System.currentTimeMillis();

            REGISTERED_CLIENT_KEYS.put("client-req-1", reg);
            System.out.println("Pre-registered SW client key: client-req-1");
        } catch (Exception e) {
            System.out.println("WARNING: Failed to pre-register SW client key: " + e);
        }
    }
    ...
    // FIX: was an empty no-op ("CORS handled only by Apache").
    // The OptionsPreflightFilter intercepts OPTIONS before the handler ever
    // runs, so CORS headers were never set on preflight responses.  We now
    // always set them here for actual POST/GET responses, and
    // OptionsPreflightFilter does the same for preflight responses.
    private static void applyMetricsCors(HttpExchange ex) {
        ...
        h.set("Access-Control-Allow-Headers", "Content-Type, X-Client-Key-Id, X-Run-Tag");
    ...
    // FIX: now sets CORS headers for cross-origin preflight requests before
    // returning the 204. Previously, it returned 204 with no CORS headers,
    // causing browsers to reject the preflight and block all cross-origin POSTs
    // (including metrics from masteroppgave2026.no → app.masteroppgave2026.no).
