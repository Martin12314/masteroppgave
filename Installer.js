...
async function fetchDNSPin() {
  console.log('[INSTALL] VERSION = 2026-01-18TXX:YYZ');
  log('[INSTALL] VERSION = 2026-01-18TXX:YYZ');
  ...
  if (!raw) {
    throw new Error('DNS pin missing');
  }
  ...
  if (parts.v !== '1') {
    throw new Error('Unsupported DNS pin version');
  }

  if (!parts.kid || !parts.sha256) {
    throw new Error('DNS pin missing kid or sha256');
  }
...
function waitForSWKeyAck(expectedKid, timeoutMs = 2000) {
...
async function main() {
  ...
  // 2) Fetch signature public key from app origin
  ...
  if (!r.ok) {
    throw new Error('Failed to fetch /sig-pub (HTTP ' + r.status + ')');
  }
  ...
  if (jwk.created > now + 60) {
    throw new Error('SIG-PUB created in the future');
  }

  if (jwk.expires < now) {
    throw new Error('SIG-PUB expired');
  }

  if (jwk.kid !== pinned.kid) {
    throw new Error(
      `KID mismatch: DNS=${pinned.kid}, SIG-PUB=${jwk.kid}`
    );
  }
